...
export function WorldMap({ state, dispatch }: { state: AdventureState; dispatch: React.Dispatch<AdventureAction> }) {
  ...
  const nextTask = finished ? worldNodes.find(node => node.chapter && !goals.includes(`chain-${node.chapter}`)) ?? worldNodes.at(-1)! : worldNodes.find(node => node.id === "market")!;
  ...
  function label(node: WorldNode) {
    ...
    if (status(node) === "locked") return node.chapter && node.chapter > 7 ? `完成第 ${node.chapter - 1} 章解锁` : "完成鉴宝后解锁";
  ...
  return <>
    <section className="world-map" aria-labelledby="map-title">
      <div className="world-viewport" style={{ transform: `translateX(${offset}px) scale(${zoom})` }}>
        ...
        {worldNodes.filter(node => node.requiredGoals.length > 0).map(node => <div key={node.id} data-fog-for={node.id} className={`map-fog region-fog ${canEnterNode(node.id, goals) ? "cleared" : ""}`} style={{ left: `${node.x - 12}%`, top: `${node.y - 12}%` }} aria-hidden />)}
      ...
      <div className="map-quest"><span className="seal-small">委托</span><div><h2>{finished ? `下一站：${nextTask.title}` : "为《江湖夜行图》挑一件工具"}</h2><p>{finished ? "沿山路修复夜行图；已完成地点可以回看，进度保存在本机。" : "环境、许可、维护线索，一个也不能只凭印象。"}</p></div><Button variant="secondary" onClick={() => { reset(); travel(nextTask); }}>回到当前任务</Button></div>
    </section>
