...
这是一个关卡的垂直切片，不是十四章桌面正式版。没有公开 Release 下载链接，也没有签名、公证或自动更新。浏览器与桌面版不自动共享进度。

当前状态请先看[验收记录](../product/PHASE_A_PROGRESS.md)：浏览器样板、macOS Apple Silicon 开发壳与独立 `.app` 已通过本机验收。Windows、Intel Mac 和干净机器下载/安装未验证。下面是构建命令，不是假公开下载入口。
