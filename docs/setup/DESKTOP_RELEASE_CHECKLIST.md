...
2026-09-11。`v0.1.1` 已上传，但维护者下载后仍被 macOS 拦截；`v0.1.2` 用于明确无会员条件下的首次放行流程，仍不宣称已完成 Apple 公证。详见[下载启动失败记录](../testing/BEGINNER_USABILITY_TEST.md)。上传成功、代码签名完整性、公证和实际启动必须分别记录。
